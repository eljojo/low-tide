...
test.describe('Concurrency and Navigation', () => {
  test('should not navigate to new job if current job is still running', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/job/${firstJobId}`));
    ...
    // EXPECTATION: We should still be on the first job because it's still running
    // CURRENT BUG: It will likely navigate to /job/${secondJobId} immediately
    await expect(page).toHaveURL(new RegExp(`/job/${firstJobId}`));
    
    // 3. Wait for first job to finish
    await expect(firstJobItem.locator('.lt-pill')).toHaveText('SUCCESS', { timeout: 10000 });

    // After it finishes, it SHOULD move to the next one (according to user requirement)
    // "only once it finishes, move into the next one that just started"
    await expect(page).toHaveURL(new RegExp(`/job/${secondJobId}`), { timeout: 15000 });
  });
});
